...
class ControlPanelDeployCdkStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    if (useInternetFacingAlb && !useTLS) {
      throw new Error(
        'Creating an internet-facing deployment requires USE_TLS, APP_URL and an ACM_CERT_ARN',
      );
    }
    ...
    if (useTLS) {
      ...
      if (typeof process.env.APP_URL === 'undefined') {
        throw new Error(
          'APP_URL must be defined and must match certificate domain (protocol will be added based on USE_TLS value)',
        );
      }
    }
...
VPC CIDR:    ${vpc.vpcCidrBlock}`);
    ...
    const appUrl = typeof process.env.APP_URL !== 'undefined' ? process.env.APP_URL : alb.loadBalancerDnsName;
